require 'test_helper'

class ConsentControllerTest < ActionDispatch::IntegrationTest
  # consent is one of the more complex items in this project. I have deemed integration test
  # would help me sleep better at night.

  test "should redirect to survey and set consent cookie after consenting" do
    assert cookies[:consent] != 'true'
    post consent_index_path(next_page_path: study_url(1))

    assert cookies[:consent] == 'true'

    assert_redirected_to study_url(1)
  end
end


# require 'test_helper'

# class ContactsControllerTest < ActionDispatch::IntegrationTest
#   setup do
#     @contact = contacts(:one)
#   end

#   test "should get index" do
#     get contacts_url
#     assert_response :success
#   end

#   test "should get new" do
#     get new_contact_url
#     assert_response :success
#   end

#   test "should create contact" do
#     assert_difference('Contact.count') do
#       post contacts_url, params: { contact: {  } }
#     end

#     assert_redirected_to contact_url(Contact.last)
#   end

#   test "should show contact" do
#     get contact_url(@contact)
#     assert_response :success
#   end

#   test "should get edit" do
#     get edit_contact_url(@contact)
#     assert_response :success
#   end

#   test "should update contact" do
#     patch contact_url(@contact), params: { contact: {  } }
#     assert_redirected_to contact_url(@contact)
#   end

#   test "should destroy contact" do
#     assert_difference('Contact.count', -1) do
#       delete contact_url(@contact)
#     end

#     assert_redirected_to contacts_url
#   end
# end

